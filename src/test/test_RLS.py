...
class TestRLS(unittest.TestCase):
    ...
    def test_fremder_mandant_kann_nicht_auf_daten_zugreifen(self):
        ...
        self.prueffirma = Mandant('Prueffirma', self.testschema)
        self.prueffirma.nutzer_anlegen('000001', 'Daten', 'Pruefer', self.testschema)

        ergebnis = self.prueffirma.get_nutzer("000001"). \
            abfrage_ausfuehren("SELECT count(*) FROM mitarbeiter", self.testschema)
        ...
        ergebnis = self.prueffirma.get_nutzer("000001"). \
            abfrage_ausfuehren("SELECT * FROM mandanten", self.testschema)
        self.assertEqual(str(ergebnis), "[(2, 'Prueffirma')]")

        # ... und seinen Nutzer
        ergebnis = self.prueffirma.get_nutzer("000001"). \
            abfrage_ausfuehren("SELECT * FROM nutzer", self.testschema)
        self.assertEqual(str(ergebnis), "[(2, 2, '000001', 'Daten', 'Pruefer')]")
